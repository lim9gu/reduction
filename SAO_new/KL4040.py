###### SAO data processing pipeline for KL4040
###### Developed by G.Lim (lim9gu@gmail.com)
###### module edited on 2020.02.29
###### run /home/lim9/anaconda3/lib/python3.7/site-packages/lgpy/SAO/KL4040.py
#=================================================================
def fileset(camera = 'KL4040'):
    """
    1. Description
...
#=================================================================
def biascom(camera='KL4040'):
    """
    1. Description 
    : This function makes a master bias image of SAO 1-m using Pyraf. Put bias images to 20XX-XX-XX/bias/ directory. Run this code on 20XX-XX-XX directory, then pyraf chdir task enter bias directory and makes process. Output image is zero.fits and it will be copied on upper directory. Due to iraf.chdir task, you should reset python when this code is finished in order to make confusion of current directory between iraf and python! 
    
    2. Usage 
    : Start on 2018-XX-XX directory. Make bias directory which contains each bias frame. Naming of each bias images should be cal*bias.fit. Then just use biascom()
...
def darkcom(camera='KL4040') :
    ...
    2. Usage
    : Start on 2018XXXX directory. Make dark directory which contains each dark frame. Naming of each dark image should be cal*dk*.fit. Then just use darkcom().
    ...
    print('Output master '+output_name+' is created.')
#=================================================================
def flatcom(flattype='sky',  camera='KL4040') :
    """
    1. Description 
    : This function makes master-normalised images of SAO 1-m using Pyraf. Put flat images to 20XX-XX-XX/flat/ directory. Run this code on 20XX-XX-XX directory, then pyraf chdir task enter skyflat, domeflat directory and makes process. Keyword 'process' was removed for KL4040. Data from KL4040 (sCMOS) need to subtract dark. Short exposured frames (2s, 3s, 4s dark) are also have hot pixels from thermal electrons, so I made bias/dark subtraction part. And then flatcombine and normalizing will be performed. Output image is nflatX.YYY.fits (X is filter and YYY is sky or dome. This function will classify each exposure of frames!) and they will be copied on upper directory. Due to iraf.chdir task, you should reset python when this code is finished in order to make confusion of current directory between iraf and python! 
    ...
    *Default configuration is skyflat.
    ...
    for i in range(len(infilter)) :
        ...
        os.system('/usr/bin/cp '+savedir+'masterflat_'+infilter[i]+'/')
...
#=================================================================
def astrometry(imlist_name='fd*.fit') :
    """
    1. Description 
    ...
    for n in range(len(addlist)):
        com='/usr/bin/solve-field '+addlist[n]+' --backend-config /usr/local/astrometry/etc/backend.cfg --cpulimit 300 --overwrite --use-sextractor  --sextractor-config '+sexconfig+' --x-column X_IMAGE --y-column Y_IMAGE --sort-column MAG_AUTO --sort-ascending --scale-unit arcsecperpix --scale-low 0.29 --scale-high 0.32 --no-remove-lines --uniformize 0 --no-plots  --new-fits a'+addlist[n]+' --temp-dir .\n'
        print(com)
...
#=================================================================
def hdrcheck(imlist_name='a*.fit', camera='KL4040'):
    """
    1. Description 
    : When performing observation, header input could be entered wrong. This issue can name the calibrated files inconsistent way for IMSNG survey. To resolve this problem, header ['OBJECT'] would be modified to the name in IMSNG target catalog (alltarget.dat) when the image center is < fov/2 of SAO KL4040. In addition, MJD will be entered, so even if input images are not IMSNG target, they should be processed by this function.
    ...
    3. History
    2018.      Firstly made.
    2020.03.01 2020.03.01 Edited for KL4040.
    """
    import os
    import sys
    import glob
...
#=================================================================
def fnamechange(imlist_name,camera='SAO_KL4040') :
    """
    1. Description 
    : Change file name of WCS solving images (afd*.fits) using naming sequence following: Calib-SAO_CCD_name-OBJECT-UTDATE-UTSTART-FILTER-EXPTIME.fits. Then the code copies changed files to IMSNGgalaxies directory.
...
#=================================================================
def filemove( password, camera='KL4040') :
    """
    1. Description 
    ...
    for n in calframes:
        ...
        if password == '':
            ...
            os.system(makedir)
            makeobsdir = 'sshpass -p "'+password+'" ssh lim9@qso.snu.ac.kr /usr/bin/mkdir -p  '+loc+galname+'/SAO/' # If IMSNG target
            os.system(makeobsdir)
            makecamdir = 'sshpass -p "'+password+'" ssh lim9@qso.snu.ac.kr /usr/bin/mkdir -p  '+loc+galname+'/SAO/'+camera
            os.system(makecamdir)
            makefildir = 'sshpass -p "'+password+'" ssh lim9@qso.snu.ac.kr /usr/bin/mkdir -p  '+loc+galname+'/SAO/'+camera+'/'+infilter+'/'
            os.system(makefildir)
            mvcommand='sshpass -p "'+password+'" scp '+ n +' lim9@qso.snu.ac.kr:'+loc+galname+'/SAO/'+camera+'/'+infilter+'/'
            mvsubtract = 'sshpass -p "'+password+'" scp hd'+n+' hc'+n+' lim9@qso.snu.ac.kr:'+loc+galname+'/SAO/'+camera+'/'+infilter+'/'
            os.system(mvcommand)
            os.system(mvsubtract)
            chmodcom = 'sshpass -p "'+password+'" ssh lim9@qso.snu.ac.kr /usr/bin/chmod 777 -R '+loc+galname+'/SAO/'
            os.system(chmodcom)
...
def gregister(imlist_name) :
    ...
    print('Done. \a')
#=================================================================
def gregister_ref(imlist_name, refim, input_list=False, large=True):
    """
    1. Description 
...
def imcopy(imlist_name, region) :
    ...
    print('Done.')
#=================================================================
def skysex(input_image, backsize=128, backfiltersize=5, mode='single'):
    """
    1. Description 
...
def hotpants_other(sciim, refim, conv='t', scale='i', ngflag=False, refpsf='', scipsf='') :
    ...
    #FWHM = 2.355 sigma
    if ngflag == False :
        com = 'hotpants -c '+conv+' -n '+scale+' -iu 5000000 -tu 5000000 -il -1000 -tl -1000 -v 0 -inim '+sciim+' -tmplim '+refim+' -outim '+outfile+' -oci '+convfile+' -ng 3 6 0.7 4 1.5 2 3.00'
    elif ngflag == True:
        sigma_template = refpsf/ 2.355
        sigma_image    = scipsf / 2.355
        #sigma_match = np.sqrt(sigma_image**2 - sigma_template**2)
        ...
        com = 'hotpants -c '+conv+' -n '+scale+' -iu 65000 -tu 65000 -il -100 -tl -100 -v 0 -inim '+sciim+' -tmplim '+refim+' -outim '+outfile+' -oci '+convfile+' -ng 3 6 '+str(0.5*sigma_match)+' 4 '+str(sigma_match)+' 2 '+str(2.*sigma_match)#+' -tg '+str(tgain)+' -tr '+str(trdnoise)+' -ig '+str(igain)+' -ir '+str(irdnoise)+' -fom v -r 10 -ft 20 -omi badmask.fits -ko 2 -ssig 3 -ks 2 -kfm 0.99 -allm -oki kernel.fits -okn -convvar' 
    print(com)
    os.system(com)
...
def main():
    ...
    import os, sys
    import glob
    from lgpy.SAO import KL4040
    from lgpy import photcom
    print('Basic reduction starts...')
    KL4040.fileset(camera='KL4040')
    KL4040.fbiascom(camera='KL4040')
    KL4040.fdarkcom(camera='KL4040')
    KL4040.fflatcom(flattype='sky',  camera='KL4040')
    KL4040.fastrometry(imlist_name='fd*.list')
    KL4040.fhdrcheck(imlist_name='a*.fit')
    KL4040.fnamechange(imlist_name,camera='SAO_KL4040')
    print('Basic reduction is finished. ')
    print('Photometry starts...')
    ...
    for i in range(len(calim)):
        ...
        band = inim.split('-')[5]
        if band == 'B' :
            photcom.run_phot(inim, 'APASS', 'SAO_KL4040', inmagkey='MAG_APER_6', inmagerkey='MAGERR_APER_6', refmagkey='B', refmagerkey='Berr', sigma_n = 5, skylim=True)
        elif band == 'V' :
            photcom.run_phot(inim, 'APASS', 'SAO_KL4040', inmagkey='MAG_APER_6', inmagerkey='MAGERR_APER_6', refmagkey='V', refmagerkey='Verr', sigma_n = 5, skylim=True)
        elif band == 'R' :
            photcom.run_phot(inim, 'APASS', 'SAO_KL4040', inmagkey='MAG_APER_6', inmagerkey='MAGERR_APER_6', refmagkey='R', refmagerkey='Rerr', sigma_n = 5, skylim=True)        
        elif band == 'I' :
            photcom.run_phot(inim, 'PS1', 'SAO_KL4040', inmagkey='MAG_APER_6', inmagerkey='MAGERR_APER_6', refmagkey='I', refmagerkey='Ierr', sigma_n = 5, skylim=True)
    os.system('gethead Cal*.fits zp zper exptime fwhm_SE lim_SE')
    KL4040.filemove('kangsun710', camera='KL4040')
