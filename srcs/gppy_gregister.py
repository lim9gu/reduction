...
def main(imkey):
    ...
    for date in datelist:
        sublist = glob.glob('fDfCalib-*-{}-*.fits'.format(date)); sublist.sort()
        refim = sublist[0]
        gregistering(sublist, refim)
        comlist = glob.glob('fDfCalib-*-{}-*_gregister.fits'.format(date)); comlist.sort()
        comlists.append(imcombine(comlist))
    #	CLEAN
    os.system('rm imcombine.list fDfCalib-*_gregister.fits')